// Types widget: type-check small AST expressions, show inferred types or errors.
...
function annotate(n) {
  // produce { node, type | error, children: [annotated] }
  if (n.t === 'Bin') {
    const lc = annotate(n.l);
    const rc = annotate(n.r);
    const r = check(n);
    return { node: n, info: r, children: [lc, rc] };
  }
  const r = check(n);
  return { node: n, info: r, children: [] };
}
...
export function initTypesWidget(root) {
  let pick = 0;
  root.innerHTML = `
    <div class="widget">
      <div class="widget-title">Walk the AST, attach types</div>
      <div class="controls" id="ty-picks">
        ${SCENARIOS.map((s, i) => `<button class="btn" data-i="${i}">${s.code}</button>`).join('')}
      </div>
      <div class="widget-stage" id="ty-stage" style="min-height:240px;"></div>
      <div class="callout" id="ty-explain"></div>
    </div>
  `;
  ...
  function render() {
    ...
    nodes.forEach((n) => n.children.forEach((c) => {
      svg += `<line x1="${xs(n.x)}" y1="${ys(n.y)+22}" x2="${xs(c.x)}" y2="${ys(c.y)-22}" stroke="var(--ink)" stroke-width="1.5"/>`;
    }));
    nodes.forEach((n) => {
      ...
      svg += `<rect x="${xs(n.x)-w/2}" y="${ys(n.y)-22}" width="${w}" height="44" rx="6" fill="${fill}" stroke="var(--ink)" stroke-width="2"/>`;
      svg += `<text x="${xs(n.x)}" y="${ys(n.y)-3}" text-anchor="middle" style="font-family:var(--font-mono);font-size:13px;font-weight:600;">${label.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</text>`;
      svg += `<text x="${xs(n.x)}" y="${ys(n.y)+14}" text-anchor="middle" style="font-family:var(--font-mono);font-size:10px;fill:${err ? 'var(--accent)' : 'var(--ink-soft)'};">${err ? '⚠ error' : ':' + n.info.type + (n.info.coerced ? '*' : '')}</text>`;
    });
    svg += `</svg>`;
    root.querySelector('#ty-stage').innerHTML = svg;

    const top = ann.info;
    if (top.error) {
      root.querySelector('#ty-explain').innerHTML = `<strong>Type error.</strong> <code style="color:var(--accent);">${top.error}</code> — the compiler refuses to emit code for this expression.`;
    } else if (top.coerced) {
      root.querySelector('#ty-explain').innerHTML = `<strong>Inferred type: ${top.type}</strong> (with an implicit int→float coercion). The starred type is where the compiler inserted a conversion.`;
    } else {
      root.querySelector('#ty-explain').innerHTML = `<strong>Inferred type: ${top.type}.</strong> Each node's type was computed from its children — no annotations needed.`;
    }
  }
