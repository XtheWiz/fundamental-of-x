import{j as p}from"./main-BMJeqOcY.js";import{L as A}from"./LegacyWidget-BJxFD61r.js";const u=[{problem:"You need to support sorting users by name, age, or signup date — and add more orderings later without touching the sort function.",options:["Strategy","Singleton","Observer"],correct:"Strategy",why:"Each ordering is an interchangeable algorithm behind a common interface. <code>sort(users, byName)</code>, <code>sort(users, byAge)</code>. Adding a new ordering = a new strategy, no edits to <code>sort</code>."},{problem:"When a chat message arrives, you need to update the unread badge, the notification dropdown, and the page title — and other parts of the app should be able to react too.",options:["Decorator","Observer","Factory"],correct:"Observer",why:"The subject (message-received) emits an event. Each interested component subscribes. Adding a new reactor = subscribe; the subject doesn't know or care."},{problem:"Different image formats (PNG, JPEG, WebP) need to be loaded — the caller shouldn't care which format, just hand back a Image object.",options:["Factory","Adapter","Observer"],correct:"Factory",why:"The Factory picks the right concrete class based on the input. The caller is shielded from the if/else over file extensions."},{problem:"Your code uses a payment gateway with method <code>charge(amountCents)</code>. You're switching to a new provider whose method is <code>process({total, currency})</code>. You don't want to rewrite every caller.",options:["Adapter","Strategy","Singleton"],correct:"Adapter",why:"Wrap the new provider in a class that exposes the old <code>charge(amountCents)</code> signature. Callers stay; the adapter translates calls under the hood."},{problem:"A request to a flaky external API: sometimes it's up, sometimes it's timing out. You want to stop hammering it after several failures and let it recover.",options:["Observer","Circuit Breaker","Decorator"],correct:"Circuit Breaker",why:'Track failure rate; after a threshold, "trip" the breaker — calls fail fast for a cool-off window before testing the upstream again. Modern resilience pattern, not in GoF.'},{problem:"You need a centralized logger that all parts of the app use, with config loaded once at startup. But the tests want to substitute a fake logger.",options:["Singleton","Dependency Injection","Factory"],correct:"Dependency Injection",why:"Singleton makes testing painful (global state). DI passes the logger in explicitly — production wires the real one; tests pass a fake. Same shared logger, no global."}];function I(a){const e=new Array(u.length).fill(null);let d=0;a.innerHTML=`
  ...
  `,a.querySelector("#qz-prev").addEventListener("click",()=>{d>0&&d--,l()}),a.querySelector("#qz-next").addEventListener("click",()=>{d<u.length-1&&d++,l()}),a.querySelector("#qz-reset").addEventListener("click",()=>{e.fill(null),d=0,l()});function l(){const s=u[d],o=e[d],n=a.querySelector("#qz-content");n.innerHTML=`
      <div class="qz-problem">${r(s.problem)}</div>
      <div class="qz-options">
        ${s.options.map(i=>`
          <button class="qz-opt ${o===i?i===s.correct?"right":"wrong":""}" data-opt="${r(i)}" ${o!==null?"disabled":""}>${i}</button>
        `).join("")}
      </div>
      ${o?`<div class="qz-why ${o===s.correct?"right":"wrong"}">${o===s.correct?"✅ Right.":`❌ The intended answer was <strong>${s.correct}</strong>.`} ${s.why}</div>`:""}
    ...
    `,n.querySelectorAll(".qz-opt").forEach(i=>i.addEventListener("click",()=>{e[d]=i.dataset.opt,l()})),a.querySelector(".widget-title").textContent=`Problem ${d+1} of ${u.length}`;const t=e.reduce((i,c,g)=>i+(c===u[g].correct?1:0),0);a.querySelector("#qz-score").textContent=`Score: ${t}/${u.length}`}function r(s){return String(s).replace(/[&<>"']/g,o=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[o])}l()}const T=[{name:"Aiko",age:29,signup:"2024-01-15"},{name:"Mateo",age:41,signup:"2023-11-02"},{name:"Priya",age:34,signup:"2025-03-08"},{name:"Kenji",age:22,signup:"2024-06-21"},{name:"Lena",age:37,signup:"2024-12-30"}],w={name:{label:"by name",fn:(a,e)=>a.name.localeCompare(e.name)},age:{label:"by age",fn:(a,e)=>a.age-e.age},signup:{label:"by signup (new→old)",fn:(a,e)=>e.signup.localeCompare(a.signup)}},D=`function sort(users, order) {
  if (order === 'name') {
...
// sort() never changes.`;function O(a){let e="name";a.innerHTML=`
    <div class="widget">
      ...
      <div class="controls">
        ...
        <div class="pill-group">
          ${Object.entries(w).map(([r,s],o)=>`
            <input type="radio" name="st-s" id="st-${r}" value="${r}" ${o===0?"checked":""}>
            <label for="st-${r}">${s.label}</label>
          `).join("")}
        </div>
      </div>
  ...
  `,a.querySelectorAll("input[name=st-s]").forEach(r=>r.addEventListener("change",s=>{e=s.target.value,d()}));function d(){const r=T.slice().sort(w[e].fn),s=a.querySelector("#st-stage");s.innerHTML=`
      <table class="st-table">
        <thead><tr><th>name</th><th>age</th><th>signup</th></tr></thead>
        <tbody>
          ${r.map(o=>`<tr><td>${l(o.name)}</td><td>${o.age}</td><td>${o.signup}</td></tr>`).join("")}
        </tbody>
      </table>
      <style>
    ...
    `}function l(r){return String(r).replace(/[&<>"']/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[s])}d()}const C=`// Publisher knows every reactor by name.
...
bus.emit('message-arrived', { from: 'Aiko', text: 'hi' });`,k=[{id:"badge",label:"Badge",icon:"🔴",describe:a=>`unread = ${a}`},{id:"dropdown",label:"Dropdown",icon:"📥",describe:(a,e)=>e?`last: "${e.text}" from ${e.from}`:"empty"},{id:"title",label:"Page title",icon:"🪟",describe:a=>`(${a}) inbox · X-sensei`}],x=["Aiko","Mateo","Priya","Kenji","Lena","Diego","Yuna","Omar"],S=["hello!","are you free?","lunch?","fyi: ship today","lol","wow","thanks!","lmk"];function j(a){const e={unread:0,last:null,enabled:{badge:!0,dropdown:!0,title:!0},log:[]};a.innerHTML=`
    <div class="widget">
      ...
      <div class="controls">
        ...
        ${k.map(r=>`<label><input type="checkbox" data-obs="${r.id}" checked> ${r.icon} ${r.label}</label>`).join("  ")}
      </div>
  ...
  `,a.querySelectorAll("input[data-obs]").forEach(r=>r.addEventListener("change",s=>{e.enabled[s.target.dataset.obs]=s.target.checked,d()})),a.querySelector("#ob-publish").addEventListener("click",()=>{const r={from:x[Math.floor(Math.random()*x.length)],text:S[Math.floor(Math.random()*S.length)]};e.unread+=1,e.last=r,e.log.unshift(`publish: { from: "${r.from}", text: "${r.text}" }`),e.log.length>6&&(e.log.length=6),d(r)}),a.querySelector("#ob-reset").addEventListener("click",()=>{e.unread=0,e.last=null,e.log=[],Object.keys(e.enabled).forEach(r=>e.enabled[r]=!0),a.querySelectorAll("input[data-obs]").forEach(r=>r.checked=!0),d()});function d(r){let s='<div class="ob-stage">';s+='<div class="ob-pub"><div class="ob-pub-icon">📨</div><div class="ob-pub-name">SUBJECT<br><small>message-arrived</small></div></div>',s+='<div class="ob-obs">',k.forEach(o=>{const n=e.enabled[o.id];s+=`<div class="ob-sub ${n?"active":"mute"} ${r&&n?"just-fired":""}">
      ...
      </div>`}),s+="</div></div>",s+=`<div class="ob-log">${e.log.map(o=>`<div>${l(o)}</div>`).join("")||'<span style="color: var(--ink-faint);">no events yet</span>'}</div>`,s+=`<style>
    ...
    </style>`,a.querySelector("#ob-stage").innerHTML=s}function l(r){return String(r).replace(/[&<>"']/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[s])}d()}const W=`// Caller switches on every type. Adding a format = edit here.
...
const req = new HttpRequestBuilder()
  ...
  `,a.querySelectorAll("input[name=fb-tab]").forEach(n=>n.addEventListener("change",t=>{e=t.target.value,d()}));function d(){const n=a.querySelector("#fb-content");e==="factory"?(n.innerHTML=`
      ...
      `,n.querySelectorAll("button[data-load]").forEach(t=>t.addEventListener("click",()=>s(t.dataset.load,t.closest(".widget-stage"))))):n.innerHTML=`
        <div class="widget-stage" id="fb-stage">${r()}</div>
        <div class="dp-grid">
          <div class="dp-side bad">
...
payments.charge(199_99);  // callers don't change`,$=[{id:"log",label:"log",prefix:"[log] req received",suffix:"[log] req done"},{id:"auth",label:"auth",prefix:"[auth] check token",suffix:"[auth] (pass)"},{id:"comp",label:"compress",prefix:"[compress] gzip on",suffix:"[compress] body compressed"}];function N(a){let e="decorator";const d={log:!0,auth:!0,comp:!0};a.innerHTML=`
  ...
  `,a.querySelectorAll("input[name=da-tab]").forEach(n=>n.addEventListener("change",t=>{e=t.target.value,l()}));function l(){e==="decorator"?r():s()}function r(){const n=a.querySelector("#da-content");n.innerHTML=`
...
`)})}function s(){const n=a.querySelector("#da-content");n.innerHTML=`
      <div class="widget-stage">
        <div class="da-flow">
...
await uow.commit();   // one transaction; all-or-nothing`;function K(a){const e={changes:[],committed:!1};a.innerHTML=`
  ...
  `,a.querySelector("#uw-add-order").addEventListener("click",()=>d("INSERT","orders",'new Order(user, "open")')),a.querySelector("#uw-add-item").addEventListener("click",()=>d("INSERT","order_items","product_id=42, qty=2")),a.querySelector("#uw-update-user").addEventListener("click",()=>d("UPDATE","users","last_order_at=NOW()")),a.querySelector("#uw-commit").addEventListener("click",l),a.querySelector("#uw-rollback").addEventListener("click",r),a.querySelector("#uw-reset").addEventListener("click",()=>{e.changes=[],e.committed=!1,s()});function d(n,t,i){e.committed||(e.changes.push({op:n,table:t,detail:i}),s())}function l(){e.changes.length&&(e.committed=!0,s())}function r(){e.changes=[],e.committed=!1,s()}function s(){let n=`
      <div class="uw-header">
  ...
  `,a.querySelectorAll("input[name=ma-p]").forEach(r=>r.addEventListener("change",s=>{e=s.target.value,d()}));function d(){const r=E[e],s=600,o=180,n={};r.diagram.forEach((c,g)=>{const b=60+g*((s-120)/Math.max(1,r.diagram.length-1));n[c.id]={x:b,y:o/2}});let t=`<svg viewBox="0 0 ${s} ${o}" width="100%" style="max-width: ${s}px">`;t+=`<style>
    ...
    </style>`,t+='<defs><marker id="ma-arr" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto"><polygon points="0 0, 8 4, 0 8" fill="var(--ink)"/></marker></defs>',r.edges.forEach((c,g)=>{const b=n[c[0]],f=n[c[1]];if(!b||!f)return;const q=n[c[0]].x<n[c[1]].x?-22:22,v=(b.x+f.x)/2,h=(b.y+f.y)/2+q;t+=`<path class="ma-edge" d="M ${b.x} ${b.y} Q ${v} ${h}, ${f.x} ${f.y}"/>`;const y=Math.max(60,c[2].length*6);t+=`<rect class="ma-edge-bg" x="${v-y/2}" y="${h-7}" width="${y}" height="14" rx="2"/>`,t+=`<text class="ma-edge-text" x="${v}" y="${h+4}" text-anchor="middle">${l(c[2])}</text>`}),r.diagram.forEach(c=>{const g=n[c.id];t+=`<rect class="ma-node" x="${g.x-50}" y="${g.y-22}" width="100" height="44" rx="6" fill="${c.color}"/>`,t+=`<text class="ma-label" x="${g.x}" y="${g.y+5}" text-anchor="middle">${l(c.label)}</text>`}),t+="</svg>";const i=`
      <div class="ma-header">
        <div class="ma-title">${l(r.label)}</div>
        <div class="ma-sub">${l(r.sub)}</div>
        <div class="ma-flow"><code>${l(r.flow)}</code></div>
      </div>
      ${t}
      <div class="ma-grid">
        <div class="ma-card">
